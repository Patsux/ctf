...
Je ne connais rien en émulation et debug de ROM gameboy, je pars donc à la
recherche d'un outil. Un critère : il doit tourner sur Linux, car je suis
venu les mains dans les poches, sans même une VM Windows.

Je trouve plusieurs outils, mais c'est bgb qui se démarque. Il a un débogueur et fonctionne avec wine sous Linux. Pour lancer l'émulation, rien de plus simple:
...
plein de points d'arrêt. Évidemment, c'est rapidement le bordel : il s'arrête
dans les routines d'affichage tout le temps et il n'y a rien d'intéressant.

Je cherche alors à remonter au code qui contient l'intelligence du programme.
...
moins j'apprends à utiliser le débogueur et je découvre que l'on peut activer un
...
Comme le programme réagit lorsque l'on appuie sur les touches, je cherche à
dans cette direction. Je finis par trouver des bouts de code où il est question du joystick. Je me mets à fouiller la doc de la gameboy pour en savoir plus et je
tombe sur une info importante ici http://www.emulatronia.com/doctec/consolas/gameboy/gameboy.txt :
...
mettre dans le registre **a**. Le problème c'est que lorsque le break se déclenche,
je lâche la touche, donc il n'arrive pas à lire la valeur de la touche ! Je mets
...
code à exécuter. Par exemple, quand j'appuie sur mon "a", le registre **a** a la
valeur 0x10 et il jump sur 0x081A:

![](img/wargames-screen-event-a.png "Routine appelé après l'appuie sur a")

Il charge l'adresse 0xCAA0 et y ajoute un offset. Il stocke la valeur
0x62 à l'adresse calculée. Un rapide coup d'oeil aux valeurs stockées dans le
tableau qui commence en 0xCAA0 et on découvre qu'il contient tout simplement le
password tapé:
...
marche ! Le password n'est pas vérifié dans l'ordre...
...
J'apprends en réalité que l'épreuve ne se terminait pas à 6h, mais
à 7h, j'ai donc pris un pause, de l'eau (si si...) avant de reprendre l'analyse.

Je comprends enfin qu'il fait un switch sur la position du caractère à tester
...
J'ai 9 caractères sur 10 quand l'épreuve se termine. Assez dégouté à ce moment...
...
PS: je crois avoir entendu qu'un bug donnait d'autres mots de passe valides.
